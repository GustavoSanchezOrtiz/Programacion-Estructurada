...
def crear_usuario():
    ...
    if nombre=="" or email=="":
        print("El nombre y el email no pueden estar vacíos. Por favor, intenta de nuevo.")
...
def iniciar_sesion():
    ...
    password=hash_password(password) #Encriptacion de contraseña
    if email=="":
        print("El email no puede estar vacío. Por favor, intenta de nuevo.")
        Funciones.EsperarTecla()
        return
    if password=="":
    ...
    if conexion and cursor:
        try:
            ...
            if usuario:
                Funciones.menu_ventas(usuario[0], usuario[1])  # ID y nombre del usuario
            else:
...
def iniciar_sesion_invitado():
    ...
    if valor_verificado: 
        ...
        Funciones.menu_ventas(valor_verificado[0][0], valor_verificado[0][1])  # ID y nombre del usuario
        Funciones.EsperarTecla()
    else:
        if conexion and cursor:
            try:
