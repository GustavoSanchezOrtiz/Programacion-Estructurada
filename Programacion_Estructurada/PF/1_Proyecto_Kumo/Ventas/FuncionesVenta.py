...
def venta_menudeo(ID_usuario):
        ...
        try:
            ...
            if sabor != "":
                ...
                if sabor >= 1 and sabor <= 8 :
                    ...
                    print(f"el sabor de {sabor}, fue añadido exitosamente")
                    Funciones.EsperarTecla()
                else:
                    print("Sabor no válido. Por favor, selecciona un número del 1 al 8.")
                    Funciones.EsperarTecla()
            elif sabor == "":
                break
        except ValueError:
    ...
    if porciones_vendidas == 0:
        print("No se han vendido porciones. No se puede registrar la venta.")
        Funciones.EsperarTecla()
        return None
    try:
...
def venta_mayoreo(ID_usuario):
    ...
    while sabor != "":
        try:
            ...
            if sabor != "":
                ...
                if sabor >= 1 and sabor <= 8 or sabor == "":
                    sabor = str(sabor)
                    if sabor == "1":
                        sabor = "Fresa"
                    elif sabor == "2":
    ...
    porciones_vendidas = len(Lista_Sabores)
    #Costo total por cada 4 litros(por porcion)
    if porciones_vendidas >= 12:
        Total = 30*2 + 18*4
    elif porciones_vendidas >= 4:
        Total = 32*2 + 20*4
    else:
        Total = 35*2 + 22*4
    Costo_de_produccion = Costo_de_produccion+(botella_litro*2 + botella_medio*4)
...
def modificar_insumo():
    try:
        ...
        sql = "SELECT ID_Producto, Nombre_producto, Costo_del_producto FROM insumos"
        cursor.execute(sql)
        ...
        for insumo in insumos:
            print(f"ID_Producto: {insumo[0]}, Nombre: {insumo[1]}, Costo: {insumo[2]}")
        Funciones.EsperarTecla()
        ...
        if id_insumo == "" or nuevo_nombre == "" or nuevo_costo == "":
            print("Los campos no pueden estar vacíos.")
            Funciones.EsperarTecla()
            return

        sql = "UPDATE insumos SET Nombre_producto=%s, Costo_del_producto=%s WHERE ID_Producto=%s"
        values = (nuevo_nombre, nuevo_costo, id_insumo)
        cursor.execute(sql, values)
        conexion.commit()
        print("Insumo modificado exitosamente.")
        Funciones.EsperarTecla()
    except Exception as e:
...
def eliminar_insumo():
    ...
    try:
        ...
        if id_insumo == "":
            print("ID de insumo no puede estar vacío.")
            Funciones.EsperarTecla()
            return
        sql = "DELETE FROM insumos WHERE ID_Producto=%s"
...
def modificar_venta(ID_usuario):
    ...
    try:
        ...
        if id_venta == "" or nuevo_total == "" or nueva_ganancia == "":
...
def importacion_a_exel():
    ...
    try:
        ...
        carpeta = documentos / "Kumo" / "exports"
        carpeta.mkdir(parents=True, exist_ok=True)

        time = datetime.now().strftime("%Y%m%d_%H%M%S")
        ruta_salida = carpeta / f"ventas_exportadas_{time}.xlsx"

        df.to_excel(ruta_salida, index=False)
        print(f"✅ Exportación completa: {ruta_salida}")

    except PermissionError:
        print("❌ No tengo permiso para escribir el archivo.")
        print("• Cierra el Excel si lo tienes abierto.")
        print("• O intenta ejecutar el programa como usuario con permisos.")
    except Exception as e:
        print("❌ Ocurrió un error al exportar a Excel.")
        print(f"Detalles: {e}")