...
def buscar_alumno(listas):
    borrar_pantalla()
    print("\t..::📂 Buscar Alumno 📂::..\n")
    nombre_a_buscar=input("Ingrese el nombre del alumno a buscar: ").upper().strip()
    if nombre_a_buscar in listas:
        for i in len(listas):
            total_de_alumnos_encontrados=0
            if nombre_a_buscar==listas[0]:
                total_de_alumnos_encontrados+=1
                renglon=listas[0]
                print(f"Alumno encontrado: {renglon[0]}")
                print(f"Calificacion 1: {renglon[1]}")
                print(f"Calificacion 2: {renglon[2]}")
                print(f"Calificacion 3: {renglon[3]}")
            print(f"Total de alumnos encontrados: {total_de_alumnos_encontrados}")
    else:
        print(f"❌El alumno {nombre_a_buscar} no se encuentra en la lista de calificaciones❌")
